...
!!! note "Permissions"
    ...
    - **Network** — used only to download live GFS weather data for cloud maps, optional extra NASA maps of the Earth and the Milky Way, and to auto-detect your location. Everything else works fully offline, including the Earth maps that ship with the add-on.
    - **Files** — used to save generated cloud maps and downloaded maps into a folder you choose.
...
!!! summary "What happens when the atmosphere is added?"
    The add-on sets up its own World for the active scene. The sky, the sun light and everything you see is rendered and kept up to date by the add-on from then on. Your exposure, white balance and viewport are left exactly as they were. `Remove Atmosphere` (at the bottom of the panel) takes it all out again.

## Choose your interface layout

At the top of the panel you can pick how much of the add-on you want to see:

- **Simple** — the everyday controls: location, time, exposure, air, clouds, moon, ground, stars and a quality preset. No science required. This is the default.
- **Scientific** — every knob there is: the physics of the air, the full cloud layers, water, the sun and planets, and all rendering settings.

The [documentation](/physical-atmosphere/documentation/) covers both, and marks what is Scientific-only.
...
- **Post Processing** — exposure and white balance. Exposure is a plain slider that starts at your scene's own value; switch the mode to `Auto` and the add-on meters the scene like a camera.
- **Atmosphere** — how clean or polluted the air is, how much haze it carries, and whether the air bends light near the horizon (`Atmospheric Refraction`).
- **Clouds** — cloud coverage, and three cloud layers you can switch on, move and thicken.
- **Moon** — the moon's rotation in Artistic mode, and how bright its sunlit side and its earthshine are.
- **Ground / Earth** — the ground's colour, city lights, terrain height and roughness. Pick `Image` and an Earth map that ships with the add-on fills the slot; the download button next to it fetches more from NASA.
- **Stars** — star brightness at night, and the Milky Way.
- **Quality** — one preset dropdown that balances quality against speed.

When you want more control (the physics of the air, full cloud layers, weather maps, water, the sun and planets), switch to **Scientific** and head to the [documentation](/physical-atmosphere/documentation/).
